...
const VERSION = 'jungle-wall-v2';
const SHELL = ['./', './index.html', './main.js', './style.css', './manifest.webmanifest', './icons/icon-192.png', './icons/icon-512.png'];
...
async function precache() {
  ...
  const code = await (await fetch('./main.js', { cache: 'no-store' })).text();
  const models = [...new Set(code.match(/\.\/[\w.-]+\.glb/g) || [])];
...
function isHeavyAsset(url) {
  return url.hostname === 'cdn.jsdelivr.net' || /\.(glb|png|webp|jpg)$/i.test(url.pathname);
}
